 ...
 */
/proc/get_path_to(path_caller, end, max_distance = 30, mintargetdist, id=null, simulated_only = TRUE, turf/exclude, skip_first=TRUE, diagonally = FALSE)
	...
	var/list/path
	var/datum/pathfind/pathfind_datum = new(path_caller, end, id, max_distance, mintargetdist, simulated_only, exclude, diagonally)
 ...
 */
#define CAN_STEP(cur_turf, next,thing,id) Toomycross(cur_turf,next,thing,id) //(next && !next.density && cur_turf.Adjacent(next) && !(simulated_only && SSpathfinder.space_type_cache[next.type]) && !cur_turf.LinkBlockedWithAccess(next,path_caller, id) && (next != avoid))
...
/datum/pathfind/New(atom/movable/path_caller, atom/goal, id, max_distance, mintargetdist, simulated_only, avoid, diagonally = 0)
