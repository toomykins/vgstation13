/********************
* Track orders made by centcom
*
* Used for the new cargo system
*********************/
var/global/current_centcom_order_id=124901

/datum/centcom_order
	var/id = 0 // Some bullshit ID we use for fluff.
	var/name = "CentCom" // Name of the ordering entity. Fluff.
...
/datum/centcom_order/New()
	..()
	id = current_centcom_order_id++
	name = command_name()

/datum/centcom_order/proc/CheckShuttleObject(var/obj/O, var/in_crate)
...
/datum/centcom_order/proc/CheckFulfilled(var/obj/O, var/in_crate)
	for(var/typepath in requested)
		if(!(typepath in fulfilled) || fulfilled[typepath] < requested[typepath])
			return 0
	return 1

/datum/centcom_order/proc/Pay()
	acct.charge(-worth,null,"Payment for order #[id]",dest_name = name)

/datum/centcom_order/proc/getRequestsByName(var/html_format = 0)
	var/manifest = ""
	if(html_format)
		manifest = "<ul>"
...
/datum/centcom_order/proc/getFulfilledByName(var/html_format = 0)
	var/manifest = ""
	if(html_format)
		manifest = "<ul>"
...
/datum/centcom_order/proc/OnPostUnload()
	return

// These run *last*.
/datum/centcom_order/per_unit
	recurring=1
	var/list/unit_prices=list()

// Same as normal, but will take every last bit of what you provided.
/datum/centcom_order/per_unit/CheckShuttleObject(var/obj/O, var/in_crate)
	if(must_be_in_crate && !in_crate)
		return 0
	if(!O)
		return 0
...
/datum/centcom_order/per_unit/CheckFulfilled()
	var/toPay=0
...
// ORDERS START HERE
//////////////////////////////////////////////
/datum/centcom_order/per_unit/plasma
	name = "Nanotrasen"
	recurring = 1
	requested = list(
...
/datum/centcom_order/department/New()
	..()
	acct = department_accounts[acct_by_string]

/datum/centcom_order/department/cargo //Orders that cargo can manage
	acct_by_string = "Cargo"

/datum/centcom_order/department/cargo/diamonds/New()
	..()
	requested = list(
...
/datum/centcom_order/department/cargo/uranium/New()
	..()
	requested = list(
...
/datum/centcom_order/department/cargo/gold/New()
	..()
	requested = list(
...
/datum/centcom_order/department/cargo/silver/New()
	..()
	requested = list(
...
/datum/centcom_order/department/cargo/phazon/New()
	..()
	requested = list(
...
/datum/centcom_order/department/cargo/clown/New()
	..()
	requested = list(
...
/datum/centcom_order/department/science //Orders that science can manage
	acct_by_string = "Science"

/datum/centcom_order/department/science/nuclear_gun/New()
	..()
	requested = list(
...
/datum/centcom_order/department/science/subspace_tunnel/New()
	..()
	requested = list(
...
/datum/centcom_order/department/medical
	acct_by_string = "Medical"

/datum/centcom_order/department/medical/kidneys/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian
	acct_by_string = "Civilian"

/datum/centcom_order/department/civilian/pie/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/poutinecitadel/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/sweetsundaeramen/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/superburger/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/turkey/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/popcake/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/bkipper/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/potentham/New()
	..()
	requested = list(
...
/datum/centcom_order/department/civilian/sundayroast/New()
	..()
	requested = list(
...
/proc/create_centcom_order(var/datum/centcom_order/C)
	SSsupply_shuttle.add_centcom_order(C)

/proc/get_potential_orders()
	var/list/orders = list()
	orders.Add(subtypesof(/datum/centcom_order/department/cargo))
	orders.Add(subtypesof(/datum/centcom_order/department/science))
	orders.Add(subtypesof(/datum/centcom_order/department/medical))
	orders.Add(subtypesof(/datum/centcom_order/department/civilian))

	return orders

/proc/create_random_order()
	var/choice = pick(get_potential_orders())
	create_centcom_order(new choice)
